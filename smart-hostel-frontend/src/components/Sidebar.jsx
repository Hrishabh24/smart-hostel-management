...
function Sidebar() {
  ...
  const menuItems = [
    ...
    { path: "/room", icon: Bed, label: "My Room" },
    { path: "/attendance", icon: QrCode, label: "Attendance" },
    { path: "/fees", icon: CreditCard, label: "Fees & Billing" },
    { path: "/complaints", icon: AlertCircle, label: "Complaints" },
    { path: "/leave", icon: UserMinus, label: "Leave Requests" },
    { path: "/support", icon: HelpCircle, label: "Support Center" },
  ...
  return (
    <div className="w-72 bg-[#131B2F]/40 backdrop-blur-xl border-r border-white/5 p-6 flex flex-col justify-between min-h-screen sticky top-0">
      ...
      <div className="space-y-4">
        <div className="p-4 bg-gradient-to-br from-blue-600/10 to-purple-600/10 border border-white/5 rounded-2xl">
          <p className="text-[10px] font-black text-blue-400 uppercase tracking-widest mb-1">Support</p>
          <p className="text-xs text-gray-400 font-medium">Need help with your stay or payments?</p>
          <button 
            onClick={() => navigate("/support")}
            className="mt-3 text-xs font-bold text-white hover:underline underline-offset-4"
          >
            Contact Desk
          </button>
        </div>
