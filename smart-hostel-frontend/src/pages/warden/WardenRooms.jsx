...
function WardenRooms() {
  ...
  const fetchRooms = async () => {
    try {
      const res = await axios.get("http://localhost:2008/warden/rooms", config);
      setRooms(res.data);
    } catch (err) {
      console.log(err);
    }
  ...
  const handleCreateRoom = async (e) => {
    ...
    setLoading(true);
    try {
      // Technically points to admin since the underlying logic to create is generic but the endpoint is currently in admin space
      // Wait, let's create a warden equivalent for POST /rooms or use the same POST /admin/rooms but change backend.
      // I added `POST /admin/rooms` but it restricts to `admin`.
      // I should update server.js to allow Warden if I add this. Let's make the endpoint POST /warden/rooms in server.js
      await axios.post("http://localhost:2008/warden/rooms", newRoom, config);
  ...
  const handleDeleteRoom = async (roomNumber) => {
    ...
    try {
      await axios.delete(`http://localhost:2008/warden/rooms/${roomNumber}`, config);
      fetchRooms();
    ...
    <>
      ...
      <div className="bg-[#131B2F]/80 backdrop-blur-md border border-white/5 shadow-[0_4px_20px_rgba(0,0,0,0.3)] rounded-lg p-6 mb-8">
        ...
        <form onSubmit={handleCreateRoom} className="flex flex-wrap gap-4 items-end">
          ...
          <button type="submit" disabled={loading} className="bg-gradient-to-br from-purple-600 to-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded font-medium transition">
